...
### HikariCP 커넥션 풀 튜닝 결과
- 풀 사이즈: 10 → 20으로 증가
- **중앙값 응답시간 21% 개선**: 1,678ms → 1,323ms
- **P95 악화**: 4,278ms → 6,301ms
- 원인: t3.micro 단일 서버에서 MySQL과 Spring Boot가 공존, 커넥션 증가 시 MySQL CPU 경합 심화
- 결론: 전용 DB 서버 환경에서는 효과적이나, 단일 서버 환경에서는 풀 사이즈 최적값이 존재함을 확인
...
- **HikariCP 튜닝**: 커넥션 풀 10 → 20으로 증가, 중앙값 응답시간 21% 개선 (1,678ms → 1,323ms). 단일 서버 한계로 P95는 악화 → 분리된 DB 서버 환경에서의 개선 방향 확인
